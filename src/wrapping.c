...
static int parsar(int argc,char *argv[],pMesh extmesh, pMesh intmesh,pSol sol) {
  ...
  if ( extmesh->ref == 0 ) {
    fprintf(stdout,"  -- EXTMESH REF ?\n");
    fflush(stdin);
    fscanf(stdin,"%d",&extmesh->ref);
  }
...
int main(int argc,char **argv) {
  ...
  fprintf(stdout,"\n  %s\n   MODULE WRAPPING-ICS : %s (%s)\n  %s\n",LS_STR,LS_VER,LS_REL,LS_STR);
  if ( info.imprim )   fprintf(stdout,"\n  -- INITIALISATION\n");
  if(! scaleMesh(&intmesh)) return (1);
  ...
  while( (count <= countmax)) {
    ...
    memset(sol.u,0,sol.dim*sol.np*sizeof(double));
    count3 ++;
    fprintf(stdout,"     - count = %d, countmax = %d, count2 = %d, count3 = %d, \n",count,countmax,count2,count3);
  }
